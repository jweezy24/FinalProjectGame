    ...
    if (isEnemy || boss) {
      for (int i = 0; i < enemies.size(); i++) {
        ...
        while (tempEnmey.alive && player.isAlive()) {
          if (isEnemy) nar.BattleLines(player, tempEnmey);
          if (boss) nar.BossRoom(player, tempEnmey);
    ...
    if (isChest) {
      ...
      if (input.toLowerCase().equals("e")) {
        for (int i = 0; i < chest.items.size(); i++) {
          System.out.println(
              "You found "
                  + chest.items.get(i).equipmentType
                  + " press i to inspect or press enter to to skip the item.");
