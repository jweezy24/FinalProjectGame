...
public class Town {
  ...
  public void insideShop(User player) {
    ...
    while (!selection.toLowerCase().equals("x")) {
      System.out.println(
          "To observe the items in the shop, press i if you would like to leave press x.");
      selection = input.Scan();
      if (selection.toLowerCase().equals("i")) {
        int[] tempItemAmmounts = new int[shopItems.length];
        for (int i = 0; i < shopItems.length; i++) {
          System.out.println(
              "We have: "
                  + shopItems[i].peek().equipmentType
                  + " X"
                  + (shopItems[i].asList().size() - 1)
                  + " at price "
                  + shopItems[i].peek().price
                  + " to buy this item press "
                  + i);
          tempItemAmmounts[i] = shopItems[i].asList().size() - 1;
        }
        ...
        while (ammount != 0) {
          if (num < shopItems.length && num >= 0) {
            ...
            if (player.money < shopItems[num].peek().price) {
              System.out.println(
                  "You don't have enough money to purchase " + shopItems[num].peek().equipmentType);
            }
